...
- iter-263 · 2026-09-27 · carrier-surface (D-239, R0 — the owner's question over the J-rows: which authored institutions in Sarrow Vale should be canonical carriers of world change, which intentionally remain narrative surfaces, and can a minimal world-track experiment distinguish the cases without a new runtime primitive): the THREE-MOVE PROBE landed as world-track law (the reader audit → the ablation arm → the injection arm, WORLD_TESTS §7's third member + WORLD_AUTHORING §7's doctrine row; every arm a scratch pack copy through the real load_pack — the lint part of the instrument; determinism twin-verified; zero core/pack changes, the instruments outside the repo) + the measured matrix over the composition witness (A1 the market's material leg ablated → the whole year diverges: no council, no vigil, the PC never caught — a LIVE carrier; A2 the economic leg → exactly one account prop, the render-read state carrier, the chest a sink with no behavioral reader; I1/I2 the council/vigil knowledge legs → +6/+23 records with the watch-briefing transfer consumer MEASURED, the epistemic band, the novelty-gradient honest boundary — rumor_told 22→22 in the co-located witness shape, the iter-262 estimate 22→25 was block-shape-dependent; I3a/I3b the market-mourns director hook (the prop trigger reading loc_malby.destroyed) → the move release MASSIVE (the mistress leaves the ashes: talks 287→2, rumors 22→2, the council pile-up 1→208 off the cold-frozen fear at the year crossing — the B2 semantics through the faction door), the ramble release MINIMAL-but-material (+5 records, rumors 22→25, one materialized scene prop — the observe resolver's canon birth); C1/C2 the calendar line's consumer routes ALL CLOSED, measured twice: the lint refuses a hooks key on the entry, the on_action scope empty over 36 turns) + the verdict PER-LEG never per-institution (the market carries all four bands; the intentional surface — the legibility of time — a VERDICT never a failure; the tale contradiction a render-side row) + four expressibility boundaries named (the calendar grammar, the flow grammar — the guild's rent honestly outlives the stalls, the ignite resolver's no-hooks surface, the absent action state block) + the J-rows re-framed by measurement (J-1 decomposed to the READER legs, the authored unit ANCHOR_REGION §6.5 at the authored band, the embodiment owner-gated each with its measured price; J-2's remainder confirmed with the refined cascade price; J-3 closed) + the owner's RU report at docs/iterations/iter-263-carrier-surface-report.md; 8 paths; 2445+9 + ruff + docguard + topology --check clean (R0 — doc-only, the D-022 exception, the owner's fresh question)
- iter-262 · 2026-09-26 · composition-diagnosis (D-238; the owner's iter-262 brief — reproduce the iter-261 J-evidence, discriminate the competing explanations, land only the warranted change): ALL J observations reproduced (the census classes, 86/287, 36/36 market days + the 12/12 fairs extension) then DISCRIMINATED — the twin-fold causal-inertness test (52 calendar turns + 5 institutional events removed → the projection byte-identical; references are cause-chain neighbors only), the static consumer enumeration (market_opens/fair_opens = tale lines only; `destroyed` read by the fire layer alone; the one materialized fire loss = the ignition affordance, spot_available door-rejects re-arson), the scratch falsifiers (a J-1 suppression gate is provably cosmetic — zero behavioral delta, the report's claimed «the market empties» consequence unreachable by it; J-3's fear>=20 urgency structurally dead — max road fear 10, the cold freeze + the decay-catchup-before-rolls law), and the J-2 refutation finding the LIVE committed bug (KI#103: pressure_pack's convene_the_bench/plead_for_coal carried authored knowledge blocks the wait resolver silently dropped — FIXED, the iter-53 hooks precedent's twin, `_wait` now mints action knowledge, empty blocks stay empty, the pressure golden regenerated on its intended 2-line delta (the two the_watch_pleads events gain their authored 6 records each — the boiler house hears the plea through the shared wall), the fresh-run guard the mutation catcher + the plea-is-audible witness the semantic pin; the other four fixtures byte-identical) + the dispositions (J-1 → the market-as-meso-carrier world-track row, owner-gated, NEVER a calendar gate; J-2 remainder → pure pack data now, owner-gated with the measured rumor cascade 22→25; J-3 → CLOSED as a non-gap) + the owner's RU report at docs/iterations/iter-262-diagnosis-report.md; 11 paths; 2445+9 + ruff + docguard + topology --check clean (R2 — one resolver's minting surface, zero new primitives, the calendar/LOD/scheduler/B2/B3/A2 untouched)
- iter-261 · 2026-09-26 · p1-composition-timing (D-237; the owner's iter-261 brief — the composition question over the parked intake-40 P1 set: P1-1 opened by the owner's explicit «prove composition» call, P1-10 = temp-1's half (b) test arm): the Province INTEGRATED witness LANDED — tests/playscripts/province_composition.json (province_pack@0.1, seed 2, existing verbs only: the lamp take → the keep arson → the failed pay-tin steal → the market arson → the year wait) + tests/test_p1_composition.py (16 tests, RELATIONAL oracles per the brief's §12 — cause→consequence per chain: the theft→knowledge→leverage→suspicion→the rotation briefing spreading the WHOLE stack to the relief→the document_check→the arrest→caught irreversible; the fire→alarm→fear→guild_councils; both griefs→the vigil ×4 incl. a YEAR-LATER re-fire off the decay-0 residue (the persistence→future-option oracle); the calendar year 36/12/4/1 + the thaw storm; the macro year's 4 account flows; the condensation; the 287-talk ambient band; the final-projection persistence; the B3 two-times discipline 383/383; the OCC miss director_0001 2520→3449; the byte-identical twin) + the cov-1 RUNTIME arm — mechanics.py census --log/--script (the A..H loss vocabulary over one run: A no pressure · B pressure without an urgency entry · C urgency/no intent with the LOD-scope and gate reasons measured from the log · D the OCC window · E quiet resolutions (the faction answers' render-side consumer) · F net-zero decay · G consumerless residue · H the full path; the director releases payload-attributed through the actions reverse index) + the P1-10 instrument — mechanics.py timing --log/--script (the two-times table: the B3 discipline block, the latency distribution + buckets, the per-family rows, the OCC misses, the compression view — THE FIRST NAMED assignment_tick CONSUMER outside the contract test) + the H1/H2 season-scale sliced pair measured (span vs 129600×wait-1: the material chain outcomes IDENTICAL (arrest/caught/council/vigil/burnouts), the crossing families invariant (A2's law at province scale), the deltas door-only — intent_rejected 20→0, checks 1→11, exactly two final-projection props) + the owner's RU report A–J at docs/iterations/iter-261-composition-report.md (the language exception owner-directed); the honest findings: the province_calendar witness provenance FIXED (a 3-step temporal STRESS shape, never integrated-world evidence), B3 consumer status UNRESOLVED→diagnostic-only, B1 NO promotion evidence, the J-gaps all pack-level (the burned market still opens 36/36; the faction answers carry no state writes; the road pressure unconsumed); 8 new mechanics tests; 13 paths; 2444+9 + ruff + docguard + topology --check clean (R2 — two periphery instrument arms + a witness script + tests + docs, zero core change, the LOG untouched, B2/B3/A2 semantics untouched)
- iter-260 · 2026-09-26 · temp1-landing (D-236 (R4) + the schema 0.3 bump; the owner's 2026-09-26 temp-1 contract pick — A2 + B2/B3, B1 deferred): the fork card's decision record landed in phases.md §6 + A2 the crossing-family slicing contract PINNED as tests/test_temp1_contract.py (20 tests: the declared tavern crossing scope count-invariant across the arms at all four seeds over the recorded surface; the minimal pair projection+fingerprint invariance with the wait-only delta; the door families' freedom BOUNDED — the seed-125 coerce flip and the seed-1001 document-check cascade + the one fingerprint divergence PINNED as the no-silent-scheduler-change guard; the seeds-42/7 agreement pinned as the honest divergence bound) + B3 the semantic-origin provenance LANDED through the existing primitive (IntentData.assignment_tick stamped at the ONE enqueue door `_enqueue_autonomous` — the based_on_event_seq precedent — flowing through `_provenance` to accepted AND rejected autonomous resolutions; player intents never carry it) + B2 UNTOUCHED (the province_calendar witness: 2288 events, all 265 talks still in the last five ticks, the origins [1800, 516600] now observable, deferral latency ≤ 518269 — `event.t` stays THE canonical timestamp) + B1 DEFERRED behind the card's §11 evidence conditions (never this row's work) + the schema bump 0.2 → 0.3 (EVENT_SCHEMA §7/§8, the cause_hook additive precedent) with ALL FIVE fixtures regenerated in the same iteration (TEST_PLAN §3 discipline: header bump + province 4 / road 1 / pressure 6 assignment_tick lines, every canonical field byte-identical to BASE — the pre/post comparison run and recorded) + the three hand-crafted version pins synced (test_smoke $id, test_semantic_diff's stale-version arm re-pointed at 0.2, test_observatory_read's expectation now derives per test_render's precedent); 12 paths; 2421+9 + ruff + docguard + topology --check clean (R4 — one additive provenance field over the existing primitive, zero runtime-semantics change, the canonical outputs verified byte-identical)
