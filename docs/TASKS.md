...
- `wb-2..wb-N` — the family's forward rows (owner-gated, each fires
  ...
  directories + clock); wb-4 the inbound gateway (INV-4's owner-gated
  exception, its own contract first); wb-5+ per the brief's §32/§46
  ladders (chat, models, inference, history, diagnostics, the CanonSim
  seam, asset vocabulary, the G1–G12 gates). Never speculative — a row
  opens only when the owner names it.
