...
- `wb-2..wb-N` — the family's forward rows (owner-gated, each fires
  ...
  wb-5+ per the brief's §32/§46 ladders (minimal application
  operations + the typed API contract; live events + reconnect;
  persistence; chat, models, inference, history, diagnostics, the
  CanonSim seam, asset vocabulary, the G1–G12 gates). Never
  speculative — a row opens only when the owner names it.
