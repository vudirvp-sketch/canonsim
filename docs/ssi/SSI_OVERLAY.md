...
| B | Topology — what actually changes together? | tests/test_architecture.py (the import boundary, D-031) | OPEN | ssi-3 (Phase 2) delivers the owner/reads/writes/emits/co-change map; a line-count snapshot is never the evidence (N018) |
| C | Complexity — how large is the semantic state space? | AGENTS.md §6 (the cap table, docguard-enforced) + core/packlint/ (the content budgets) + the (tick, sub_order, actor_id) queue key's bounded order | PARTIAL | the doc/pack budgets executable at HEAD; the core/ complexity budget rides ssi-3's map |
...
| SSI-N018 | no-architecture-health-from-snapshot | scripts/docguard.py | OPEN | the phase-evidence lint is armed (Phase 2/4 rows must name co-change/trajectory evidence); the FULL trajectory audit fires with ssi-3 — until then every external-analysis number stays a hypothesis, never material_gap |
...
| 2 | ssi-3 | the ownership/topology audit (core/, workbench/) | owner-gated | co-change/trajectory evidence over git history — NEVER a line-count snapshot (N018); re-verifies the external analysis's hypotheses (loop.py ~1714 lines etc.); confirmed = the material_gap that opens ssi-5, refuted = that scope shrinks |
| 3 | ssi-4 | the workbench/application/inference.py split | owner-gated | the ssi-3 ownership map (the semantic owners decide the split, never an external template) |
...
| 5 | ssi-6 | the canonical read seam (core ↔ workbench read-side) | owner-gated | INV-4's sanctioned-module pattern («exactly N modules», D-193/D-201/D-208's own form) |
