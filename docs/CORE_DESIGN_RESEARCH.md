...
| agent-memory-atlas (§5) | 151-system memory meta-survey: 7 capability marks (correction/tombstone 8/151, trust 24/151, scope 85/151) + 22 named patterns with carriers and counterexamples; "correction is the phase that goes unbuilt"; companion clients hold the hysteresis patterns the rest of the field lacks | chat-assistant/coding-agent corpus — nothing faces a deterministic-canon constraint; no benchmark for forgetting; retrieval-quality monoculture (paradigms 1–6 solve IR, only paradigm 8 solves governance) | phase-1 scene ledger (D-048: trust states, rejected-value tombstone, scope-as-key, hysteresis→pinning, evidence-before-belief; negative: TTL decay, chained summarization, vectors over dynamic facts) + phase-4 memory checklist + forbidden-assertion test discipline (`docs/ref/agent_memory_atlas.md`) |
