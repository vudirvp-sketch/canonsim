...
|---|---|---|---|
| RNG-1 | INV-2 "one `random.Random(seed)` instance" vs Brogue two-stream / DCSS N-stream discipline | **RngBank**: a single authority in `core/rng.py` holding named streams, each deterministically derived from the master seed (stable hash of `f"{seed}:{stream}"`). `substantive` = canon stream; `cosmetic` = render-only. Cosmetic rolls can never desync canon replay; `audit()` scope guard (DCSS `ASSERT_stable`); draw counter on substantive = replay fingerprint for T1 | iter-1 |
...
| BRIEF-1 | Generative Agents retrieval vs letta block manager vs D-018 sensory-delta brief vs live-char voice isolation vs lorebook injection | **The block pipeline**: typed blocks with hard token budgets — [directives] [scene delta (sensory emitters, O(perception))] [recalled facts] [scheduled static lore] [voice exemplars near the context end] [active options]. Retrieval split: dynamic facts = `known_by` + tick window + event weight (**never vector search** — `VISION.md` §5); static lore = FTS5 keyword first, sqlite-vec optional. Injection scheduling grammar: depth / probability / cooldown / sticky / range-cascade | phase 1 |
