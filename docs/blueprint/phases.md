...
10. **The depth battery's intake-27 sharpenings** (the level-design
   consolidation, D-173; the card's own recalibration law applies):
   the pack-gate evaluation now asks (a) **name the reader** —
   every new event type cites its read surface (a projection
   consumer, a knowledge channel, a door check, a tale line, or a
   hook; zero readers = functionally dead — the 48-turns law,
   D-170's census law gone event-side); (b) count
   **differently-priced options**, never labeled options (static
   prices = the fictitious-choice symptom, the A–H B row's own
   measured shape); (c) the **top-LOD read survives the blur** —
   the critical path (who / where / what changed / why) reads at
   the coarsest LOD. The authoring-side companions held in the
   intake-27 block: the realization table (why exists / which
   layer / linked to what / what the reader understands + the
   removal test) and the motivation scope question (which player
...
**The level-design & worldbuilding consolidation routed (intake-27,
iter-140, D-173 — the owner's «проанализируй документ и определи
что из него можно полезного перенять в проект и почему... где
прокатит "междоменная трансплантация принципов"» research call over
the uploaded `level-design-consolidated.md` — a consolidated
conspectus of twelve GDC / Konsoll / Level-Design-Lobby talks
spanning level design, worldbuilding, player motivations,
procedural generation, production, and cursed problems; doc-only,
zero code, zero corpus price — the D-155..D-171 intake precedent;
the research file external per the convenience-copy law, repository
evidence authoritative; the intake-24 transplantation posture:
extract the mechanism source-free, never copy the form — the
sources are spatial/rendering talks, the target a deterministic
text simulation with no camera and no renderer). Verified BEFORE
working at HEAD 1b7666a — 1754+1 green, ruff clean, Python
3.12.14, the numbers identical after.** The document's question
for the repo: which of its principles transplant into a simulation
core whose "level" is the event log, whose "player path" is the
intent door, and whose "renderer" is the brief — and what is
already owned law. The verdict: **PARTIALLY CONFIRMED — the
doctrine is overwhelmingly owned** (the strongest cross-domain
confirmation family since intake-16: a spatial-geometry discipline
and an event-sourcing simulation arriving at the same laws —
budgets, subtraction, LOD, authored-over-generated, data-driven
behavior), **the residue five compact instruments and one
proposal**, all routing to standing rows; zero build-grade items.

**The verdict table** (the document's sections → candidate
principles, source-free; the five-question form — principle /
form / quality / transfer / combined — compressed per the D-170
precedent; form-match alone never sufficient):

| # | The principle (source-free mechanism) | The standing form at HEAD | Quality / transfer | Disposition |
|---|---|---|---|---|
| 1 | figure–ground + negative space (Brown): attention is budgeted; contrast decides parse; good design subtracts | BRIEF-1's hard per-block budgets + whole-block eviction + `[truncated:N]`; CHRON-1's per-cause-chain pruning; LOD-1's coherent LODs; AGENTS §6.1's substance filter — the repo's own subtraction law | the mechanisms owned (FACT); the missing piece the compact DIAGNOSTIC — the blur/squint test: strip the read to its top LOD, the critical path must survive | ADOPT — instrument 3 |
| 2 | discoverability (Deck): unfound content is functionally nonexistent; information-density tuning (all visible devalues exploration, nothing visible kills motive) | the knowledge system (channels `saw/heard/told/inferred`, fidelity, `known_by`) the NPC half; the brief's `known_by` filter the PC half; D-170's census law ("motivation needs a runtime consumer") the discipline; the 48 turns the measured instance | both halves owned, the instance measured; the unifying EVENT-side law is new: an emitted event with zero read surfaces is functionally dead | ADOPT — instrument 1 |
| 3 | zones, vistas, non-game space (Kirill/Pears): functional differentiation; life beyond the frame at minimal detail | depth-3's scene LOD; the ambient noise floor; the cold tiers / macro aggregates; depth-2 lazy detail; the worldgen chronicle horizon | owned — the vista principle IS the cold-tier law (the world lives beyond the frame as aggregates) | CONFIRMED owned |
| 4 | space reuse / value drift (Serra/Taylor): the same geometry reads differently over time | state = fold(log): the burned market → the grief wake; the calendar cadences re-value locations; weather's seasonal layer | owned — event sourcing's own shape (geometry constant, meaning folded) | CONFIRMED owned |
| 5 | priced freedom (Serra): choice without differential price is fictitious; density of interactions over size of freedom; readable causality (signal before threat — the siren law); the player as tipping point | intake-18 D/F PROVEN (route = time/exposure/information; irreversibility); L8's immediate price = the siren law's own shape; L9's intersections; the A–H B row measured PARTIAL (options 1-of-6, "the lattice prices static") | the principle owned; the SHARPENING new: count differently-PRICED options, never labeled options — static prices are the fictitious-choice symptom the B row already measured | ADOPT — instrument 2 |
| 6 | emotion-first authoring (Taylor): decide the feeling, then the means; goals say WHAT never HOW; parallel goals; never punish improvisation | DIR-1's pacing clock (the L4D donor); AP-9 spines (want/need/flaw/cause — the character-side form); the intent door (improvisation is data, attempts are facts, rejections carry reasons) | owned in mechanism; the feeling-first ORDER is the pack-candidate card's proof-value question (the falsifiable proposition stated first) | CONFIRMED owned |
| 7 | player motivations as scope control (Deck): which motivation does this serve? no answer → scope creep; content volume ≠ content value; opt-in affordances per zone | the admission lint's teleology gate — the machine half (dead action types, orphan entities, declared-but-unused templates); L1; intake-18/25's compellingness fence (INV-4 — not decidable in track A) | the machine half owned; the player-facing half deliberately fenced — the motivation lens is presentation-1's selection vocabulary, never track-A law | PARK — rides instrument 4's card line |
| 8 | realism vs realization (Edwards): the minimal property set that convinces FOR THIS GAME; five ingredients (familiarity / hinted systems / cultural evidence / logical consistency / topology); build from causes, not objects; backfilling discipline (nothing added to fill emptiness — every element answers "why is this here") | the worldgen pass ladder (layers begetting layers — Azgaar's pipeline); place-1; L7 (causes recorded at write time); name-1 + D-119 prohibitions + the Morrowind row (cultural evidence); the admission lint (backfilling's machine half); the Qud D-130 row (authored pillars over generated surface) | the ingredients owned as mechanisms; the AUTHORING-side verification table new — the machine half cannot check "what the reader understands" | ADOPT — instrument 4 |
| 9 | PCG is design (Pittman/Qud/Richter/Serra): rules-constraints-probabilities, never "the algorithm invented it"; authored + procedural; randomness must change decisions; vary the designed space | the packs ARE rules/probabilities (INV-3, PACK-1); D-082/D-134 state-coupled randomization (the Fallout anti-row); worldgen = ordered passes over authored pillars; the cosmetic stream (the player completes texture) | owned as standing law — four independent talks converging on the repo's own law family | CONFIRMED owned; the catalog rows record it |
| 10 | Eldritch's measured authoring (Pittman): prescribed rooms (reserve the special, build around); module statistics (run the generator 10k, size the pool); theme tags with foreign-module landmarks; placement laws from playtests (loot → dead-ends, traps → corridors, enemies → junctions) | place-1's claim↔exits; the condensation anchors (the reserved-structure shape); the golden corpora + the balance harness (the distribution instruments exist) | the topology-aware hook distribution new as a PROPOSAL: the site graph's degree classes each deserve their hook families — reasons-to-go distributed by topology | PROPOSAL (below) |
| 11 | Qud's abstraction mountain: history as abstract pairs reified through artifacts; default/random when no abstract input; one step's output the next's input; population tables (pick one / pick each) | the worldgen pass ladder; the chronicle horizon reified through tales / cause trees; lazy detail; the templates + rules weighted picks (pick-one ≈ choose, pick-each ≈ the per-entry urgency rolls); L7 fences the runtime outcome-first form — legal at genesis only (the cause tree IS generated history) | owned; the talk's own "choose the outcome, backfill the explanation" trick is legal exactly where the repo allows it (genesis) and forbidden where L7 rules (runtime) — the fence confirmed from the outside | CONFIRMED owned |
| 12 | Control's representation switching (Richter): multiple representations per object; each granularity its cheapest form; material-driven behavior (the record carries the reaction, never the code); degradation strategy from day one; generic procedural + hand-authored exceptions | LOD-1; L12; INV-3/PACK-1 (material-driven's exact shape); the cold tiers (never simulate equally); AP-1 budgets; the brief's per-block zonal budgeting | the strongest single cross-domain confirmation: a destruction pipeline and a narrative sim sharing the representation-switching law; the one new nuance — transition masking (particles cover the representation switch) = the narrator covering LOD seams | CONFIRMED owned; the masking nuance parked to presentation-1 (this block, no separate fixation) |
| 13 | production data quality (Richter): naming conventions as API; wrong metadata multiplies through the system; auto-checks, never artist attention; playtests read WHY not WHAT | the pack-lint family (D-152 — the philosophy verbatim: machine checks over attention); JSON-schema packs; the corpus replay + M1–M5 (log-only, never by feel); intake-26's claim packets | owned — the pack pipeline IS the naming-conventions-as-API law | CONFIRMED owned |
| 14 | cursed problems (Jaffe): promise pairs (designer + player-inferred); the both-promises test (can one artifact fulfill both without becoming another game?); the broken-promise zone on the golden path; four sacrifice types — Barrier (remove) / Gate (make expensive) / Carrot (re-point the incentive) / S'more (make it the game); partial sacrifices legal; cursed contradictions as genre sources | NO such vocabulary in the repo (verified at HEAD) — but the resolved cases read in it: the canon/voice split = Barrier (the LLM never writes canon); sycophancy cured by mechanics = Carrot (rolls decide persuasion — the incentive moved); the improvisation straitjacket = the named partial sacrifice (VISION §6's honest limits); the SoW fence = the standing both-promises question | genuinely new — the protocol for the "we cannot fix X" debates the SoW horizon will generate; prevents the feature→tweak→tuning spiral on contradictions built into the premises | ADOPT — instrument 5 |

**The five adopted instruments** (each the smallest form that
survives counterexamples; every one names its consumer, none is
law until that consumer uses it):

1. **The reader law** (Deck × Haggis × the census): an emitted
   event with zero read surfaces — no projection consumer, no
   knowledge channel, no door check, no tale line, no hook — is
   functionally dead; emission alone is a write to /dev/null. The
   census law's event-side twin, cross-domain confirmed from the
   player-facing side. The pack-gate question: **name the reader
   of every new event type**. Consumers: F4's promotion events
   (pack-1 / pack-4); the cadences' decision half — arming ONE
   consumer on a market/fair cadence IS this law applied (the
   BUILD question iter-139 routed, unchanged).
2. **The priced-option question** (Serra): the option surface
   counts **differently-priced options** (risk / exposure / time /
   information deltas), never labeled options — static prices are
   the fictitious-choice symptom (the B row's own measured shape:
   "the lattice prices static"). The future pack gate's B-row
   measurement form. Consumers: the depth battery at the pack
   gates; res-1's derived price spreads the dynamic half's
   standing owner.
3. **The top-LOD readability question** (Brown): blur the read to
   its coarsest LOD (the tale line, the scene card) — the critical
   path (who / where / what changed / why) must survive; detail
   that decorates only the fine LOD is budget, never figure.
   Consumer: the depth battery's read-surface arm; the cheapest
   possible brief/chronicle check — no instrument owed.
4. **The realization table** (Edwards): the pack author's
   per-element checklist — **why exists / which layer / linked to
   what / what the reader understands**, plus the removal test
   (minimal viable realization = what stops working if this
   element is removed). The admission lint is the machine half;
   this is the authoring half the lint cannot check. PARKED: the
   pack slots (pack-1 / pack-3 / pack-4) read it at their
   openings; the scaffold's editing map cites it at its next
   touch. One card line carries the motivation scope question
   with it (Deck): "which player motivation does this pack serve —
   no answer → scope creep" (presentation-1's lens, INV-4-fenced).
5. **The sacrifice protocol** (Jaffe): for the "we cannot fix X"
   family — write the promise pair (the designer's promise + the
   player-inferred promise), run the both-promises test (can one
   artifact fulfill both without becoming another game?), and if
   cursed choose the sacrifice TYPE — Barrier / Gate / Carrot /
   S'more — and NAME THE PRICE. The repo's resolved cases re-read
   as its precedents (Barrier: the canon/voice split; Carrot:
   mechanics-cured sycophancy; partial sacrifice: the
   improvisation straitjacket, VISION §6). PARKED: the SoW
   integration (bg-6), engine-1 / presentation-1, companion-1 —
   the debates the horizon generates; a debate form, never a
   gate, never a spec.

**The proposal** (recorded, never law until a pack arms it):
**topology-aware hook distribution** — roads-1's generated graph
yields degree classes over the sites (junction / through-route /
dead-end); Eldritch's playtest-derived placement laws transplant
as the reasons-to-go question: dead-end sites deserve the
reward-class hooks, through-sites the hazard/ambient class,
junctions the social/NPC class (the hook families are pack data —
place-style gates per class, never new machinery). Consumers: the
cadences' decision half, roads-1's pass, the F4 pack slots. The
counterexample it must survive: a pack whose sites are all one
class (the tavern — one room) — the question degenerates
gracefully to "no classes, no law".

**Refused at the door** (owned, fenced, or D-024): no
spatial-composition machinery (framing devices, leading lines,
camera height — renderer-side, presentation-1's, and the repo has
no camera; the transferable vocabulary — card_markers, the
weight/size reads — already owned); no motivation-model law
(INV-4 — compellingness not decidable in track A; the lens parks
with instrument 4); no new named-principles document (D-024 —
this block is the one-pass form; the source stays external per
the convenience-copy law); no cursed-problem runtime / spec /
check (a debate form only); no topology-hook arming NOW (the
owner-gated rows own the when); Kirill's eight methods and
Totten's documentation culture fully owned as confirmations (LOD,
packs, principle-then-example, D-022's pattern-not-code) — no
residue.

**Micro-drifts**: none repo-facing — the document is a conspectus
of external talks, not a repo claim set (unlike intake-26). One
caution recorded: its Qud section describes the talk's village
generator, not this worldgen — the resemblance (pass ladder,
population tables) is the transplant working, and the repo's L7
fence is STRICTER than the talk's outcome-first trick at runtime;
nothing else drifts.

Landed: D-173 + this block + the consult card's item 10 +
