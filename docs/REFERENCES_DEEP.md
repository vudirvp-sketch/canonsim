...
| ref-21 | Kurvitz worldbuilding corpus (the owner's consolidated research upload) | solo (owner-requested fresh external source — intake-31, iter-158; D-189) | the generator library: prohibition propagation, function removal, residue lifecycle, meso coupling, social carriers, humor generator → the W4 operator set + the W5 test operationalizations (world track, never runtime) |
| ref-22 | Vantiel RPG rules corpus (the owner's consolidated research handoff upload) | solo (owner-requested fresh external source — intake-32, iter-159; D-190) | the separations donor: ten distinctions already owned by the substrate + the model-boundary consult residue (the re-expansion law + the staged-interpretation sketch → presentation-1, owner-gated, never runtime) |
...
| ref-21 | Kurvitz worldbuilding corpus | `docs/ref/kurvitz.md` | proprietary (design patterns only) | world track (W4/W5 authoring consult; D-186/D-189) | the generator-library donor: the function-loss / residue-lifecycle / meso-assembly / crisis-probe operators + the humor generator + the carrier mechanism, the do-not-import list binding — detail: the ref file | done (iter-158) |
| ref-22 | Vantiel RPG rules corpus | `docs/ref/vantiel.md` | unverified (no license in the supplied archive — pattern only) | presentation-1/engine-1 consult, owner-gated (D-190) | the separations donor: ten substrate-owned distinctions + the model-boundary residue (the re-expansion law, the staged-interpretation sketch, the deadband-beats-hysteresis note) — detail: the ref file | done (iter-159) |
