...
| `scripts/` | persisted generation/analysis scripts (`AGENTS.md` §9 — Script Persistence Rule): `balance_harness.py` (iter-6: the 1000-sim distribution harness for KI#4 close; outputs `output/balance_<N>_seed<S>_<on|off>.txt` — gitignored runtime artifact, reproducible from the seed range) | landed iter-6 |
