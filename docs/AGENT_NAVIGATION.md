...
| `brief/` | the phase-1 mediator input: `assembler.py` (BRIEF-1 — the six-block pipeline: directives / scene delta / recalled facts / scheduled lore / voice exemplars / active options; two-level token budgets, whole-block eviction, `[truncated:N]` markers; pure functions of the log, zero RNG — `docs/BRIEF_SPEC.md` owns the contract) | landed iter-8 |
...
| `tests/` | test suite | smoke + architecture fitness, T0, minimal T1/T2, core units, loop e2e (iter-1); intent units, the 12 actions e2e, scheduler DAG, transition units, INV-3 stoplist (iter-2); T3 blind-NPC + knowledge/transfer/expectation suites, crime/rotation/OCC suites, iter-3 pack lint (iter-3); director + urgencies + states suites, arrest resolution updated, iter-4 pack lint (iter-4); tracery engine + chronicle + CLI suites incl. T1-chronicle and the directors A/B wiring (iter-5); the metrics module + the iter-6 T1 fixture-regeneration guard + the T8 single-factor A/B suite (iter-6); the brief assembler suite — byte-identity, beat-window mirror, budgets, eviction, brief pack lint (iter-8); the bg-1 parsing-core regression suite — sanitize/recover/census/audit + the UNDOCUMENTED marker + the measured in-flight-record truncation behavior on a tiny synthetic DF-like XML (iter-8g, extended bg-1); the bg-1 SQLite-sink suite — typed cores/EAV, participant index, membership/parent links, generic records, truncation policy (both modes), content determinism, the bg-3 consumer query (bg-1) |
| `tests/fixtures/` | committed golden fixtures for T0/T1 (log lines, byte-identical run outputs) | `plumbing_smoke_seed42.jsonl` (iter-1; env-pinned — header records the Python version; iter-4/5/6 kept byte-identical — the 58-tick scenario crosses no beat) |
