...
| `scripts/` | operator tooling (CLI-class, D-046): `balance_harness.py` (iter-6: the 1000-sim distribution harness for KI#4 close; harness contract owner `docs/TEST_PLAN.md` §6; outputs `output/balance_<N>_seed<S>_<on|off>.txt` — gitignored runtime artifact, reproducible from the seed range) | landed iter-6 |
