...
| `core/` | engine: `schema` (mini-validator, D-032), `rng` (RngBank), `ids`, `clock`, `queue`, `log` (the only canon-write path), `fold`, `pack` (loader + lint), `intent` (front-door machinery: preconditions, checks, OCC, knowledge resolution), `resolvers` (the 12-action registry), `transitions` (the generic layer engine — fire as pack data), `scheduler` (the system-pass DAG), `loop` (tick driver + playscript runner) | iter-1/2 landed |
| `sim/systems/` | the 8 systems (`MVP_SCOPE.md` §5) — declared as pack data (`rules.json` `systems`, DAG-ordered) as of iter-2; the first system code (knowledge/relations passes) lands iter-3 | skeleton restored iter-0d |
| `content/tavern_pack/` | the setting as data | v0.1 (all 12 actions executable: resolver keys, events, preconditions, knowledge templates, checks, transitions/fire layer); loaded + linted by `core/pack.py` |
...
| `tests/` | test suite | smoke (iter-0d) + architecture fitness, T0, minimal T1/T2, core units, loop e2e (iter-1); intent units, the 12 actions e2e, scheduler DAG, transition units, INV-3 stoplist (iter-2); T3–T8 grow iter-3+ |
