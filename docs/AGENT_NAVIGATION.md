...
| `brief/` | the phase-1 mediator circuit (LLM-free half): `assembler.py` (BRIEF-1 — the six-block pipeline: directives / scene delta / recalled facts / scheduled lore / voice exemplars / active options; two-level token budgets, whole-block eviction, `[truncated:N]` markers; pure functions of the log, zero RNG — `docs/BRIEF_SPEC.md` owns the contract) · `validator.py` (VALID-1 — the proposal→check→commit gate: the closed proposal document, honest verdicts `supported/contradicted/insufficient_data`, ExpectedVersion OCC, fact-transaction pass-through, the ≤2-regens protocol, golden-set plumbing; pure functions of (proposal, log, pack) — `docs/VALIDATION_SPEC.md` owns the contract) | landed iter-8; +validator iter-9 |
...
| `docs/BRIEF_SPEC.md` | brief assembly contract (the trigger fired at phase-1 start — `SPECS_BACKLOG.md` sketch row): block pipeline, budgets, eviction, render format, just-in-time deferrals | written iter-8 |
