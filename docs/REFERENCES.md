...
| Robert Kurvitz — the worldbuilding lectures (the owner's consolidated research corpus) (D-186/D-189) | worldbuilding as a GENERATOR LIBRARY, never a canon layer: prohibition propagation, negative space as function removal, residue lifecycle, meso coupling nodes, social carriers, humor as scarcity/prohibition generator → the W4 operator set + the W5 test operationalizations (WORLD_AUTHORING §5/§7/§8/§19, WORLD_TESTS §3, WORLD_WORKPLAN §6); the do-not-import list binding (races, name catalogues, humor tables, thresholds, one-to-one institutions). Deep dive: `docs/ref/kurvitz.md` (ref-21) |
| Vantiel RPG rules (the owner-supplied `Vantiel_RPG_Rules_Clean.zip`, license unverified — pattern only) (D-190) | an LLM-centric AI-GM ruleset as a SEPARATIONS donor, never a subsystem donor: truth retention ≠ recall salience, fact ≠ interpretation, emotion ≠ factual memory, relationship magnitude ≠ history, failure ≠ punishment, recovery ≠ undo, knowledge ≠ accessibility, context bundle ≠ memory truth, journal ≠ canon, world intervention ≠ narrative override — every distinction already owned by the substrate (re-verified at HEAD, intake-32); the fresh residue = the presentation-1 consult material (the re-expansion law + the staged-interpretation sketch, parked behind the owner gate). Deep dive: `docs/ref/vantiel.md` (ref-22) |
...
| The Sims (GDC 2018, M. Brown) (D-156) | the emergent-storytelling survey — confirmation row: promise trees = the hook buffer (seeded future possibility, D-005); inverse autonomy = the panic/wariness gates + the urgency weight modifiers (pressure narrows behavior); fear trees = the suspicion pair axes escalating by family; hierarchy-creates-meaning = the LOD ladder; needs-as-pressure = the AP-9 spine |
| Slime Rancher (GDC 2017, Popovich) (D-156) | a thousand tiny tales — confirmation row: needs as narrative engines = the want/need tension (AP-9's own field); breadcrumbs = the knowledge-channel substrate (sightings/rumors/tellings over real canon); anthropomorphic inference = the narrator's licensed read-side (L6 — behavior is coherent and explainable, interpretation never becomes canon) |
| The Shrouded Isle (GDC 2019, Kim) (D-156) | emergent tragedy — confirmation row: harm emerges from locally rational decisions = D-005's own law (the crime ladder's shape — a steal FAILURE walks the world to the boss beat); thematic contradiction as declared pressure patterns = the D-134 pack-law family (never bonuses, REFERENCES' WH40k grammar); tragedy needs memory = L7 + `provenance.cause_hook` + the chronicle's cause chains |
| Heaven's Vault (GDC 2018, Ingold) (D-156) | the dynamic detective story — confirmation row: authored content + procedural glue = the pack/worldgen split (D-142's admission law owns the boundary); multi-route discovery (a missed clue compensated elsewhere) = since-1's family (the re-encounter delta — the standing owner-gated row); automated narrative testing integrated into the writing workflow = the testproto protocol (TEST_PLAN §8, D-098) + the corpus discipline |
| Procedural Narrative Generation (GDC 2017, Cardona-Rivera & Martens) (D-156) | the survey's triad — confirmation row: simulation-centric vs plot-centric vs discourse = D-005 + DIR-4 (the consequence planner + entropy pacing IS drama management); data recombination over inventing content = the brief's read-side recombination (recalled_facts/present_entities over the log, O(relevance)); the missing bridge is discourse = the brief itself (the POV projection — world state → epistemic projection → narrator document) |
| Dan Taylor, *Ten Principles for Good Level Design* (D-173) | confirmation + transplant: goals say WHAT never HOW = the intent door's own law (improvisation is data, attempts are facts); difficulty as risk/reward routes, never a menu = the door's rolls decide, the player picks exposure; the emotion-first authoring order = the proof-value question's shape (the falsifiable proposition first) |
| Jim Brown (Epic), *The Importance of Nothing: Negative Space in Level Design* (D-173) | figure–ground: attention budgets, contrast, subtraction — the brief's per-block budgets + whole-block eviction and the chronicle's per-cause-chain pruning confirmed; the squint test transplants as the top-LOD readability question (the depth battery's read-surface arm — the intake-27 block, instrument 3) |
| Max Pears, *Conversing with Players: Spatial Composition* (D-173) | spatial composition as subconscious communication — the weight/size/entrance-width reads = the card_markers + scene-line vocabulary already owned; the framing/camera half is renderer-side (presentation-1's; the repo has no camera) |
| Aubrey Serr, *Designing Radically Non-Linear Single Player Levels* (D-173) | the priced-choice law: freedom without differential price is fictitious; density of interactions over size of freedom (L9's own shape); the player as tipping point (the F3 outcome-divergence form); the B-row sharpening — count differently-priced options, never labels (the intake-27 block, instrument 2) |
| David Pittman, *Procedural Level Design in Eldritch* (D-173) | the measured-authoring family: PCG is design (rules/constraints/probabilities), prescribed rooms, module statistics (run the generator, size the pool), playtest-derived placement laws — the topology-aware hook distribution PROPOSAL rides it (the site graph's junction/through/dead-end classes × the hook families — the intake-27 block) |
| Johannes Richter (Remedy), *Procedural Destruction in Control* (D-173) | representation switching: multiple representations per object, each granularity its cheapest form, material-driven behavior, degradation strategy from day one, generic + hand-authored exceptions — L12/LOD-1/INV-3's strongest cross-domain confirmation; the transition-masking nuance parked to presentation-1 |
| Raylene Deck, *Designing Game Worlds for Different Player Motivations* (D-173) | the discoverability law (unfound content ≈ nonexistent) = the reader-names-the-event law (the intake-27 block, instrument 1 — the census law's event-side twin); content volume ≠ content value; the motivation scope question (presentation-1's lens, INV-4-fenced) |
| Kate Edwards, *A Geographer's Guide to Building Game Worlds* (D-173) | realism vs realization; minimal viable realization; build from causes, not objects (the worldgen pass ladder + L7); the backfilling discipline (the admission lint the machine half; the realization table the authoring half — the intake-27 block, instrument 4) |
| Alex Jaffe, *Cursed Problems in Game Design* (D-173) | the sacrifice protocol: promise pairs (designer + player-inferred), the both-promises test, Barrier/Gate/Carrot/S'more with the price named — the repo's resolved cases re-read as precedents (Barrier: canon/voice split; Carrot: mechanics-cured sycophancy; partial: the improvisation straitjacket); PARKED for the SoW debates (the intake-27 block, instrument 5) |
| Josh Sawyer, *Gods and Dumps* (PoE attribute tuning, GDC) (D-191) | the numbers-perception donor — confirmation row: the viable band = the pack lint's dead-data refusal + D-019's measured thresholds; loss aversion + neutral point + base-and-modifiers UI + naming-as-signal = the outcome-perception card's numbers half (presentation-1 consult, parked — `docs/ref/game_design_talks.md`) |
| Sid Meier, *The Psychology of Game Design* (GDC keynote) (D-191) | the NEW domain of the D-156/D-173 families: outcome-perception psychology — expectation bands over naked odds, the invisible-roll paranoia law (cause + avoidance path on every failure), surprises outside fundamentals, the Covert Action center-of-gravity (the brief's own bounded blocks), the imagination economy (L6/mode A's family); the source's dice-bending fixes REFUSED (INV-2 — the invariant is perceived fairness, bought only at the render layer) |
| Battle Mode (Stardock), *the micromanagement term* (video essay) (D-191) | confirmation row, all three halves already law: term precision = D-024 + the terminology fence; friction-from-insignificant-management = the fill law's dropped count + mech-2's attention budget; verify-the-complaint-first = D-175 measurement-before-mechanism |
| Soren Johnson, *Old World design postmortem* (GDC) (D-191) | the richest confirmation batch + three adoptions: undo-because-deterministic = INV-2/T2; no-cheats asymmetric AI = the one-id intent door; infinite tooltips = mechanics.py; known-denominator thresholds = the world track's store budgets; events-offer-not-negotiate = the door's attempts-are-facts; ADOPTED: the anti-arbitrage spread (the Resource open question's donor — exchange lossy by construction, mechanism never the 2× number), the turnover question (the ECS cure → the function-loss probe aimed at holder mortality, WORLD_AUTHORING §5), the weak-coupling authoring law (pack-3's event-family material) |
| Herbert Wolverson, *Procedural Map Generation Techniques* (talk) (D-191) | direct-and-verify = the worldgen's superior own form (MST connectivity by construction + the reachability/dead-arming lints — never generate-then-fix); the Dijkstra-map design applications (Hot Path placement, least-accessible value, key-provably-before-lock) = the road-traffic rider's placement pair, parked beside the intake-27 topology proposal; Voronoi-metric/two-layer-noise REFUSED (saturated: ref-8/ref-9 + the LOD ladder) |
