...
price zero by construction, the arming = depth-5b). chron-2 the
history bridge landed iter-87 (D-121: the DF legends shape — every
history event carries two distinct region participants (the run's
anchor's draw inherited by its members, the DF collection's role
fields) + one site place; the war→battle→episode hierarchy as a
PACK-DECLARED collection vocabulary (`chronicle.collections`, L10 —
the tiers root-first, the kinds ⊆ the closed HISTORY_KINDS); the
cause TREE (L7 — a member chains to its nearest lower-tier
...
depth-3 the scene LOD landed iter-91 (D-125, D-116's W2 consumer
head — the macro clock's FIRST consumer): `core/lod.py` the three
zones (ACTIVE the PC's location per-beat, WARM its exits at the macro
crossings, COLD riding the turn's census alone — `cold_npcs`, the
D-112 counts-for-populations shape), the decay/urgency walks gained
the zone filter, the director stays global (the story layer); the
one-gate law — the LOD engages exactly when the macro clock is armed,
the unarmed pack is the one-scene world (the v0.1 bytes, zero corpus
price, zero re-pins; KI#83 the entities-lint KeyError leak found +
fixed the same iteration).
depth-6 factions with goals landed iter-92 (D-126 — the W2 row after
depth-3: small-formula dynamics, the KeeperRL rebellion precedent —
a ratio and a threshold, never a psychology engine): `core/factions.py`
the KeeperRL formula (the affected fraction of the membership,
per-cent floored, against the pack-declared threshold, ramping to
the pack's ceiling; the deadband at-or-below the bar; D-006 holds —
the members' LIVE per-entity axes, never a stored group score), the
group entity kind (`entities.json::groups`, D-112's one id — the
faction acts through the intent door, actor = the entity id; the
anchor scopes the scene LOD's zones, presence views never list it),
the `faction:<group>:<kind>` roll stream (the D-079 family's fifth
...
name-1 the name generator landed iter-96 (D-131 — the D-079 family
law's SIXTH member `name:<npc>`): `core/names.py::materialize_name`
the lazy door (the scene-detail twin — ONE birth (npc, `name`,
None -> drawn), first-commit-wins, the bounded collision walk
against the entity namespace — the output-namespace law), the
profiles `rules.json::names` (the culture-keyed phonotactic pools,
the Azgaar nameBase shape + the CK3 culture keying; the ref-19
dives `docs/ref/ck3.md` + `docs/ref/azgaar_names.md`), the npc
`generated_name` declaration (mutual exclusion with the authored
`name`, the reachability law), THE CONSUMER depth-7's condensation
(the name births paired with the membership births, the outcome's
`names` key), the fold-first read surface (iter-96a/KI#84: every
